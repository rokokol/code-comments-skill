...
and belongs in a repository's own gate
  ...
  check-comments.sh [-C DIR] --sentences [N] [PATH...]
...
and is skipped, as is hardware-configuration.nix and any file whose first three lines carry
a "generated by" or "do not edit" banner: those comments were composed where the generator
is, and an edit here is undone by its next run
...
CANON
  ...
  # A generator's output is read the same way as a vendored copy: the comment was composed
  # where the generator is, so a shorter one written here is gone on its next run
  d=$(copy generated-skip)
  printf '# Generated by generate.sh from palette.json — do not edit\nbase00 = "#222222" # TODO: name this one\n' \
    >"$d/theme.yaml"
  expect_quiet "$d" marker "a TODO in a file that says a program wrote it"
...
grep -E '\.(nix|sh|bash|py|yml|yaml|conf|sieve)$' "$work/files.raw" |
  ...
  grep -vxFf "$work/vendored" >"$work/files.named" || :
# A file that opens by saying a program wrote it is that program's output: its comments
# were composed where the generator is, and shortening one here is undone by the next run.
# The banner is looked for in the first three lines, which is where every generator this
# family has met puts it, and matched however the sentence is worded around the two words
: >"$work/files"
# `<<<` rather than a pipe into grep -q: grep stops at its match, and head feeding a reader
# that has gone would die of SIGPIPE, which pipefail makes the status of a test that answered
while IFS= read -r f; do
  opening=$(head -n 3 "$f" 2>/dev/null) || opening=""
  grep -qiE '^[[:space:]]*(#|//|/\*|--|;)[[:space:]]*.*(generated by|do not edit|automatically generated)' \
    <<<"$opening" ||
    printf '%s\n' "$f" >>"$work/files"
done <"$work/files.named"
...
# --sentences is a measurement, not a rule. The register's other halves — the voice, the
# noun clusters, a word in its plain sense — are a reading job, and a length on its own
# reported on every run would be a warning nobody acts on today, which teaches a reader to
# read past warnings. Asked for, it answers; unasked, it says nothing
if [[ -n "$sentences" ]]; then
  awk -F'\t' -v LIMIT="$sentences" -v ROOT="$root/" '
    function rel(p) { sub("^" ROOT, "", p); return p }
    # An empty comment line ends a paragraph, and so a sentence: joining across one welds
    # the last words of a paragraph to the first of the next and reports the pair as one
    function flush(   i, n, parts, sentence, words, w, count, j) {
      if (body == "") return
      n = split(body, parts, /[.?;] +/)
      for (i = 1; i <= n; i++) {
        sentence = parts[i]
        count = 0
        w = split(sentence, words, /[ \t]+/)
        # A token with no letter or digit is punctuation standing alone — a spaced em dash,
        # an operator inside an inline expression — and a register counts words, not glyphs
        for (j = 1; j <= w; j++) if (words[j] ~ /[0-9A-Za-z]/) count++
        if (count > LIMIT) {
          printf "%s:%s: %s words: %s\n", rel(file), first, count, sentence
          over++
        }
        total++
      }
    ...
    {
      key = $2 "\t" $4
      if (key != seen) { flush(); seen = key; file = $2; first = $3 }
      text = $12
      sub(/^[ \t]+/, "", text)
      if (text == "") body = body ". "
      else body = body (body == "" ? "" : " ") text
    }
    END {
      flush()
      # Flushed first, or the summary lands in the middle of a finding: the two streams are
      # buffered apart and a terminal shows them in whichever order they arrive
      fflush()
      printf "check-comments: %d sentence(s), %d over %d words\n", total, over, LIMIT > "/dev/stderr"
      exit(over > 0)
    }
  ' "$work/facts"
  exit $?
